...
- **Trang có sẵn**: teams/[teamId], projects, inbox, phases, task detail.
- **Data**: Toàn bộ từ `mock-data/` (tasks, teams, projects, phases…). Không có backend/API.
...
| ------------------------- | -------------------------------------------------------------- |
| `stores/tasks.ts`         | `GET /api/tasks`, Supabase `task` table                        |
