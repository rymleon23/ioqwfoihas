...
- Mỗi task có acceptance criteria rõ ràng; test song song cùng code.
...
- `main`: production branch, merge sau khi staging passed QA và checklist release.
- `staging`: stage trung gian, nhận merge từ feature branch để chạy CI/E2E.
- Feature branch đặt tên `feature/<module>-<slug>` (vd. `feature/p1-triage-inbox`).
- Commit message tuân thủ Conventional Commits; Husky commit-msg hook chạy `pnpm commitlint --edit "$1"`.
- Trước khi release: merge staging → main bằng PR, gắn tag version, cập nhật CHANGELOG.
- Sau mỗi sprint: dọn nhánh feature đã merge, đồng bộ staging từ main.
